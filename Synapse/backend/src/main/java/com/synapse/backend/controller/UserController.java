...
import com.synapse.backend.dto.FeedbackRequest;
...
public class UserController {
    ...
    public ResponseEntity<Users> updateUser(@PathVariable String email, @RequestBody Users updatedUser) {
        ...
        // Update the user's fields if they are not null or invalid (e.g., age should
        // not be negative)
    ...
    public ResponseEntity<Object> getFeedbackByEmail(@PathVariable String email) {
        ...
        if (feedback.isPresent()) {
            ...
            // Return empty structure for new users instead of 404
            Map<String, Object> response = new HashMap<>();
            response.put("email", email);
            response.put("scores", new HashMap<>());
            return ResponseEntity.ok(response);
        }
    }
    ...
    public ResponseEntity<?> getUserName(@PathVariable String email) {
        ...
        if (userOpt.isPresent()) {
            ...
            response.put("profilePicture", user.getProfilePicture()); // Assuming this stores profile picture URL or
                                                                      // binary data
